...
const ToDoTask: React.FC<ToDoTaskProps> = ({ task, onRemove, onToggleStatus, onEditTask }) => {

  
  // Modify the Date Time format to a nicer one
  ...
  return (
    <div className="bg-neutral my-2 rounded-lg p-2 text-neutral-content navbar">
      {/** Left side of the Task - Edit Button + Title */}
      <div className="flex-1">
        <EditTask onEditTask={editTask} task={task}/>
        <h3 className=''>{task.title}</h3>
      </div>
      {/** Right side of the Task - Date, CheckBox, Delete button */}
